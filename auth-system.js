...
(function() {
    ...
    function handleGoogleSignIn(response) {
        if (response.credential) {
            // Decode JWT token (simplified - in production, verify with backend)
    ...
    window.loginWithGoogle = function() {
        if (typeof google !== 'undefined' && google.accounts) {
            ...
            const authUrl = `https://accounts.google.com/oauth/authorize?client_id=${GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=openid%20email%20profile&access_type=offline`;
            window.location.href = authUrl;
        }
    };

    window.loginWithApple = function() {
        // Apple Sign-In implementation (requires Apple Developer account)
        showSuccess('Apple ile giriþ yapildi! (Simülasyon)');
        ...
        modalContent.innerHTML = `
            ...
            <form id="registrationForm" class="space-y-4">
                <div class="grid md:grid-cols-2 gap-4">
                    <input type="text" placeholder="Ad" required class="luxury-input w-full px-4 py-3 rounded-lg">
                    <input type="text" placeholder="Soyad" required class="luxury-input w-full px-4 py-3 rounded-lg">
                </div>
                <input type="email" placeholder="E-posta" required class="luxury-input w-full px-4 py-3 rounded-lg">
                <input type="password" placeholder="Þifre" required class="luxury-input w-full px-4 py-3 rounded-lg">
                <input type="password" placeholder="Þifre Tekrar" required class="luxury-input w-full px-4 py-3 rounded-lg">
                <div class="flex items-center space-x-2">
            ...
            <div class="mt-6 text-center text-sm text-gray-600">
                Zaten hesabiniz var mi? <a href="#" onclick="showLogin()" class="text-black hover:underline">Giriþ Yap</a>
            </div>
        `;
    ...
    window.showLogin = function() {
        ...
        modalContent.innerHTML = `
            ...
            <div class="text-center text-sm text-gray-600">
                Hesabiniz yok mu? <a href="#" onclick="showRegistration()" class="text-black hover:underline">Kayýt Ol</a>
            </div>
        `;
